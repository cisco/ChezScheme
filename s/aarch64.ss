...
(define-registers
  ...
  (allocable
    ...
    [     %r14                  #f 14]
    [     %r15                  #f 15]
    ;; %lr is trashed by 'c' calls including calls to hand-coded routines like get-room
    [     %lr                   #f 30]
    )
  (machine-dependent
    [%jmptmp %argtmp            #f 10]
...
(module (md-handle-jump ; also sets primitive handlers
  ...
  ;; `define-instruction` doesn't try to cover `ur`
  ...
  (let ()
    (define pick-asm-op
      (lambda (op info)
        (case op
          [(load-double)  (gen-fl-load op info #t)]
          [(store-double) (gen-fl-store op info #t)]
          [(load-single)  (gen-fl-load op info #f)]
          [(store-single) (gen-fl-store op info #f)])))

    (define-instruction effect (load-double store-double load-single store-single)
      [(op (x ur) (y ur) (z imm-constant))
       (let ([n (nanopass-case (L15d Triv) z [(immediate ,imm) imm])])
         (if (eq? y %zero)
             `(asm ,info ,(pick-asm-op op info) ,(%mref ,x ,y ,n))
             (let ([u (make-tmp 'fl1)])
               (seq
                `(set! ,(make-live-info) ,u (asm ,null-info ,(asm-add #f) ,x ,y))
                `(asm ,info ,(pick-asm-op op info) ,(%mref ,u ,%zero ,n))))))]
      [(op (x ur) (y ur) (z ur))
       (let ([u (make-tmp 'fl2)])
         (seq
          `(set! ,(make-live-info) ,u (asm ,null-info ,(asm-add #f) ,x ,z))
          `(asm ,info ,(pick-asm-op op info) ,(%mref ,u ,y 0))))]))

  #;
  (define-instruction effect (load-double)
    [(op (x ur) (y ur) (z imm-constant))
     (let ([n (nanopass-case (L15d Triv) z [(immediate ,imm) imm])])
       `(asm ,info ,(gen-fl-load op info #t) ,(%mref ,x ,y ,n)))]
    [(op (x ur) (y ur) (z ur))
     (let ([u (make-tmp 'fl2)])
       (seq
        `(set! ,(make-live-info) ,u (asm ,null-info ,asm-add ,x ,z))
        `(asm ,info ,(gen-fl-load op info #t) ,(%mref ,u ,y 0))))])

  #;
  (define-instruction effect (store-double)
    [(op (x ur) (y ur) (z imm-constant))
     (let ([n (nanopass-case (L15d Triv) z [(immediate ,imm) imm])])
       `(asm ,info ,(gen-fl-store op info #t) ,(%mref ,x ,y ,n)))]
    [(op (x ur) (y ur) (z ur))
     (let ([u (make-tmp 'fl2)])
       (seq
        `(set! ,(make-live-info) ,u (asm ,null-info ,asm-add ,x ,z))
        `(asm ,info ,(gen-fl-store op info #t) ,(%mref ,u ,y 0))))])

  #;
  (define-instruction effect (load-single)
    [(op (x ur) (y ur) (z imm-constant))
     (let ([n (nanopass-case (L15d Triv) z [(immediate ,imm) imm])])
       `(asm ,info ,(gen-fl-load op info #f) ,(%mref ,x ,y ,z)))]
    [(op (x ur) (y ur) (z ur))
     (let ([u (make-tmp 'fl2)])
       (seq
        `(set! ,(make-live-info) ,u (asm ,null-info ,asm-add ,x ,z))
        `(asm ,info ,(gen-fl-load op info #f) ,(%mref ,u ,y 0))))])

  #;
  (define-instruction effect (store-single)
    [(op (x ur) (y ur) (z imm-constant))
     (let ([n (nanopass-case (L15d Triv) z [(immediate ,imm) imm])])
       `(asm ,info ,(gen-fl-store op info #f) ,(%mref ,x ,y ,n)))]
    [(op (x ur) (y ur) (z ur))
     (let ([u (make-tmp 'fl2)])
       (seq
        `(set! ,(make-live-info) ,u (asm ,null-info ,asm-add ,x ,z))
        `(asm ,info ,(gen-fl-store op info #f) ,(%mref ,u ,y 0))))])

  (let ()
    (define pick-asm-op
      (lambda (op info)
        (case op
          [(load-single->double)  (values (gen-fl-load op info #f) (asm-fl-cvt 'single->double info))]
          [(load-double->single)  (values (gen-fl-load op info #t) (asm-fl-cvt 'double->single info))]
          [(store-single->double) (values (gen-fl-store op info #t) (asm-fl-cvt 'single->double info))])))

    (define-instruction effect (load-single->double load-double->single store-single->double)
      [(op (x ur) (y ur) (z imm-constant))
       (let ([n (nanopass-case (L15d Triv) z [(immediate ,imm) imm])])
         (let-values ([(loader/storer converter) (pick-asm-op op info)])
           (if (eq? op 'store-single->double)
               (seq
                `(asm ,info ,converter)
                (if (eq? y %zero)
                    `(asm ,info ,loader/storer ,(%mref ,x ,y ,n))
                    (let ([u (make-tmp 'fl1)])
                      (seq
                       `(set! ,(make-live-info) ,u (asm ,null-info ,(asm-add #f) ,x ,y))
                       `(asm ,info ,loader/storer ,(%mref ,u ,%zero ,n))))))
               (seq
                (if (eq? y %zero)
                    `(asm ,info ,loader/storer ,(%mref ,x ,y ,n))
                    (let ([u (make-tmp 'fl1)])
  ...
  #;
  (define-instruction effect (load-double->single)
    [(op (x ur) (y ur) (z imm-constant))
     (let ([n (nanopass-case (L15d Triv) z [(immediate ,imm) imm])])
       (seq
        `(asm ,info ,(gen-fl-load op info #t) ,(%mref ,x ,y ,n))
        `(asm ,info ,(asm-fl-cvt 'double->single info))))])
  ...
  (define-instruction effect (store-single->double)
    [(op (x ur) (y ur) (z imm-constant))
     (let ([n (nanopass-case (L15d Triv) z [(immediate ,imm) imm])])
       (seq
        `(asm ,info ,(asm-fl-cvt 'single->double info))
        `(asm ,info ,(gen-fl-store op info #t) ,(%mref ,x ,y ,n))))])
  ...
  ;; tranfer bits only
  (define-instruction value (double->int)
    [(op (x ur) (y ur))
     `(set! ,(make-live-info) ,x (asm ,info ,asm-double->int ,y))])
  ...
  (define-instruction effect save-flrv
    [(op) `(asm ,info ,(asm-push-multiple (list %Cfpretval)))])

  (define-instruction effect restore-flrv
    [(op) `(asm ,info ,(asm-pop-multiple (list %Cfpretval)))])
...
(module asm-module (; required exports
                     ...
                     asm-logtest asm-fl-relop asm-relop asm-push-multiple
                     ...
                     gen-fl-load gen-fl-store asm-fl-cvt asm-flt asm-double->int asm-int->double
                     asm-trunc asm-lock asm-lock+/- asm-cas asm-fence
                     asm-flop-2 asm-flsqrt asm-c-simple-call
                     asm-return asm-c-return asm-size
                     asm-enter asm-foreign-call asm-foreign-callable
                     asm-debug asm-rp-header asm-data-label
                     asm-read-counter
                     asm-inc-cc-counter
  ...
  (define asm-size
    (lambda (x)
      (case (car x)
        [(asm aarch64-abs aarch64-jump aarch64-call) 0]
        [(long) 4]
        [else 8])))
        ...
        (cond
          [(ax-reg? dest)
           (record-case src
             ...
             [(literal) stuff
              (ax-mov64 dest 0
                (asm-helper-relocation code* (cons 'aarch64-abs stuff)))]
             [(disp) (n breg)
              (cond
                [(aligned-offset? n)
      ...
      (lambda (code* src1 src2 dest)
        (Trivit (src1 src2 dest)
                (let ([disp (constant flonum-data-disp)]
                      ...
                      [flreg2 `(reg . ,%flreg2)]
                      [flreg3 `(reg . ,%flreg3)])
                  (emit ldurfi flreg1 src1 disp
                        (emit ldurfi flreg2 src2 disp
                              (let ([code* (emit sturfi flreg3 dest disp code*)])
                                (case op
                                  [(fl+) (emit fadd flreg3 flreg1 flreg2 code*)]
                                  [(fl-) (emit fsub flreg3 flreg1 flreg2 code*)]
                                  [(fl*) (emit fmul flreg3 flreg1 flreg2 code*)]
                                  [(fl/) (emit fdiv flreg3 flreg1 flreg2 code*)]
                                  [else (sorry! who "unrecognized op ~s" op)])))))))))

  (define-who asm-fl-cvt
    (lambda (op info)
      (lambda (code*)
        (let ([flreg `(reg . ,(info-loadfl-flreg info))])
          (case op
            [(single->double)
             (emit fcvt.s->d flreg flreg code*)]
            [(double->single)
             (emit fcvt.d->s flreg flreg code*)]
            [else (sorry! who "unrecognized op ~s" op)])))))
    ...
    (lambda (code* src dest)
      (Trivit (dest src)
              (let ([disp (constant flonum-data-disp)]
                    [flreg1 `(reg . ,%flreg1)]
                    [flreg2 `(reg . ,%flreg2)])
                (emit ldurfi flreg1 src disp
                      (emit fsqrt flreg2 flreg1
                            (emit sturfi flreg2 dest disp code*)))))))
  ...
  (define asm-flt
    (lambda (code* src dest)
      (Trivit (dest src)
              (let ([disp (constant flonum-data-disp)]
                    [flreg1 `(reg . ,%flreg1)])
                (emit scvtf flreg1 src
                      (emit sturfi flreg1 dest disp code*))))))

  (define gen-fl-load
    (lambda (who info double?)
      (lambda (code* mem)
        (Trivit (mem)
                (let ([flreg `(reg . ,(info-loadfl-flreg info))])
                  (record-case mem
                       [(disp) (imm reg)
                        (if double?
                            (cond
                             [(aligned-offset? imm)
                              (emit ldrfi flreg (cons 'reg reg) imm code*)]
                             [else
                              (safe-assert (signed9? imm))
                              (emit ldurfi flreg (cons 'reg reg) imm code*)])
                            (cond
                             [(aligned-offset? imm 2)
                              (emit ldrfsi flreg (cons 'reg reg) imm code*)]
                             [else
                              (safe-assert (signed9? imm))
                              (emit ldurfsi flreg (cons 'reg reg) imm code*)]))]
                       [(index) (n ireg breg)
                        (cond
                         [(fx= n 0)
                          (if double?
                              (emit ldrf flreg (cons 'reg ireg) (cons 'reg breg) code*)
                              (emit ldrfs flreg (cons 'reg ireg) (cons 'reg breg) code*))]
                         [else
                          (sorry! who "cannot handle indexed fp src ref")])]
                       [else (sorry! who "unsupported addressing mode")]))))))
                  ...
                  (record-case mem
                    [(disp) (imm reg)
                     (if double?
                         (cond
                          [(aligned-offset? imm)
                           (emit strfi flreg (cons 'reg reg) imm code*)]
                          [else
                           (safe-assert (signed9? imm))
                           (emit sturfi flreg (cons 'reg reg) imm code*)])
                         (cond
                          [(aligned-offset? imm 2)
                           (emit strfsi flreg (cons 'reg reg) imm code*)]
                          [else
                           (safe-assert (signed9? imm))
                           (emit sturfsi flreg (cons 'reg reg) imm code*)]))]
                    [(index) (n ireg breg)
                     (cond
                      [(fx= n 0)
                       (if double?
                           (emit strf flreg (cons 'reg ireg) (cons 'reg breg) code*)
                           (emit strfs flreg (cons 'reg ireg) (cons 'reg breg) code*))]
                      [else
                       (sorry! who "cannot handle indexed fp dest ref")])]
                    [else (sorry! who "unsupported addressing mode")]))))))

  ;; move bit patterns between fp and int regs
  (define asm-double->int
    (lambda (code* dest src)
      (Trivit (dest src)
              (emit fmov.f->g dest src code*))))
  (define asm-int->double
    (lambda (code* dest src)
      (Trivit (dest src)
  ...
  (define (asm-multiple regs pop? code* one two one-fl two-fl)
    (define fp-argument-regs  (lambda () (vector %Cfparg1 %Cfparg2 %Cfparg3 %Cfparg4
                                                 %Cfparg5 %Cfparg6 %Cfparg7 %Cfparg8)))
    (define fp-reg?
      (lambda (reg)
        (memq reg (append (list %flreg1 %flreg2 %flreg3)
                          (vector->list (fp-argument-regs))))))
    (safe-assert (or (andmap fp-reg? regs)
                     (andmap (lambda (r) (not (fp-reg? r))) regs)))
    (let ([sp `(reg . ,%sp)])
      (let loop ([regs regs] [code* code*])
        (cond
         ...
         [(null? (cdr regs))
          (let ([reg (car regs)])
            (if (fp-reg? reg)
                (one-fl sp (cons 'reg reg) code*)
                (one sp (cons 'reg reg) code*)))]
         [pop?
          (let ([reg1 (car regs)]
                [reg2 (cadr regs)])
            (if (and (fp-reg? reg1) (fp-reg? reg2))
                (two-fl sp (cons 'reg reg1) (cons 'reg reg2) (loop (cddr regs) code*))
                (two sp (cons 'reg reg1) (cons 'reg reg2) (loop (cddr regs) code*))))]
         [else
          (let ([reg1 (car regs)]
                [reg2 (cadr regs)])
            (loop (cddr regs)
                  (if (and (fp-reg? reg1) (fp-reg? reg2))
                      (two-fl sp (cons 'reg reg1) (cons 'reg reg2) code*)
                      (two sp (cons 'reg reg1) (cons 'reg reg2) code*))))]))))
  ...
  (define asm-library-jump
    (lambda (l)
      (asm-helper-jump '()
                       `(aarch64-jump ,(constant code-data-disp) (library-code ,(libspec-label-libspec l))))))

  (define asm-library-call
    (lambda (libspec save-ra?)
      (let ([target `(aarch64-call ,(constant code-data-disp) (library-code ,libspec))])
  ...
  (define asm-c-simple-call
    (lambda (entry save-ra?)
      (let ([target `(aarch64-call 0 (entry ,entry))])
  ...
  (define asm-direct-jump
    (lambda (l offset)
      (asm-helper-jump '() (make-funcrel 'aarch64-jump l offset))))

  (define asm-literal-jump
    (lambda (info)
      (asm-helper-jump '()
                       `(aarch64-jump ,(info-literal-offset info) (,(info-literal-type info) ,(info-literal-addr info))))))
  ...
  (define asm-get-tc
    (let ([target `(aarch64-call 0 (entry ,(lookup-c-entry get-thread-context)))])
  ...
  (define asm-activate-thread
    (let ([target `(aarch64-call 0 (entry ,(lookup-c-entry activate-thread)))])
      (lambda (code* dest . ignore)
        (asm-helper-call code* target #t))))

  (define asm-deactivate-thread
    (let ([target `(aarch64-call 0 (entry ,(lookup-c-entry deactivate-thread)))])
      (lambda (code* . ignore)
        (asm-helper-call code* target #f))))

  (define asm-unactivate-thread
    (let ([target `(aarch64-call 0 (entry ,(lookup-c-entry unactivate-thread)))])
  ...
  (module (asm-foreign-call asm-foreign-callable)
    ...
    (define fp-reg?
      (lambda (reg)
        (memq reg (append (list %flreg1 %flreg2 %flreg3)
                          (vector->list (fp-argument-regs))))))
    (define gp-reg? (lambda (reg) (not (fp-reg? reg))))
      ...
      (lambda (regs e)
        ...
        (with-output-language (L13 Effect)
                              (let ([helper
                                ...
                                (helper (filter (lambda (r) (not (fp-reg? r))) regs)
                                        (helper (filter (lambda (r) (fp-reg? r)) regs)
                                                e))))))
    ...
    ;; same type and offset, since the AArch64 ABI
    ...
    (define categorize-arguments
      (lambda (types varargs-after)
        (let loop ([types types] [int* (vector->list (int-argument-regs))] [fp* (vector->list (fp-argument-regs))]
                   ;; varargs currently not supported
    ...
    (define memory-to-reg
      (lambda (ireg src src-offset size unsigned? tmp)
        (safe-assert (not (eq? ireg src)))
        (with-output-language (L13 Effect)
                              (let loop ([ireg ireg] [src-offset src-offset] [size size] [unsigned? unsigned?])
                                (case size
                                  [(8) `(set! ,ireg ,(%mref ,src ,src-offset))]
                                  [(7 6 5)
                                   (%seq
                                    ,(loop ireg (fx+ src-offset 4) (fx- size 4) #t)
                                    ,(loop tmp src-offset 4 #t)
                                    (set! ,ireg ,(%inline sll ,ireg (immediate 32)))
                                    (set! ,ireg ,(%inline + ,ireg ,tmp)))]
                                  [(3)
                                   (%seq
                                    ,(loop ireg src-offset 2 #t)
                                    ,(loop tmp (fx+ src-offset 2) 1 #t)
                                    (set! ,tmp ,(%inline sll ,tmp (immediate 16)))
                                    (set! ,ireg ,(%inline + ,ireg ,tmp)))]
                                  [else
                                   `(set! ,ireg ,(case size
                                                   [(1) `(inline ,(make-info-load (if unsigned? 'unsigned-8 'integer-8) #f) ,%load ,src ,%zero (immediate ,src-offset))]
                                                   [(2) `(inline ,(make-info-load (if unsigned? 'unsigned-16 'integer-16) #f) ,%load ,src ,%zero (immediate ,src-offset))]
                                                   [(4) `(inline ,(make-info-load (if unsigned? 'unsigned-32 'integer-32) #f) ,%load ,src ,%zero (immediate ,src-offset))]
      ...
      (with-output-language (L13 Effect)
                            (letrec ([load-double-stack
                                      (lambda (offset)
                                        (lambda (x) ; unboxed
                                          (%seq
                                           (inline ,(make-info-loadfl %flreg3) ,%load-double ,x ,%zero ,(%constant flonum-data-disp))
                                           (inline ,(make-info-loadfl %flreg3) ,%store-double ,%sp ,%zero (immediate ,offset)))))]
                                     [load-single-stack
                                      (lambda (offset)
                                        (lambda (x) ; unboxed
                                          (%seq
                                           (inline ,(make-info-loadfl %flreg3) ,%load-double->single ,x ,%zero ,(%constant flonum-data-disp))
                                           (inline ,(make-info-loadfl %flreg3) ,%store-single ,%sp ,%zero (immediate ,offset)))))]
                                     [load-int-stack
                                      (lambda (offset size)
                                     ...
                                     [load-boxed-single-reg
                                      (lambda (fpreg fp-disp)
                                        (lambda (x) ; address (always a var) of a float
                                          `(inline ,(make-info-loadfl fpreg) ,%load-single ,x ,%zero (immediate ,fp-disp))))]
                                     [load-int-reg
                                      (lambda (ireg)
                                        (lambda (x)
                                          `(set! ,ireg ,x)))]
                                     ...
                                     [compute-stack-argument-space
                                      ...
                                      ;; arguments that the callee expects. So, calculate how much the callee should expect.
                                      (lambda (cats)
                                                                 ...
                                                                 (cond
                                                                  ...
                                                                  [double?
                                                                   (%seq ,(loop (cdr fp*) (fx+ offset 8))
                                                                         (inline ,(make-info-loadfl (car fp*)) ,%store-double ,tmp ,%zero (immediate ,offset)))]
                                                                  [else
                                                                   (%seq ,(loop (cdr fp*) (fx+ offset 4))
                                                                         (inline ,(make-info-loadfl (car fp*)) ,%store-single ,tmp ,%zero (immediate ,offset)))])))]
                                                            [else
                              ...
                              (lambda (info)
                                ...
                                (let* ([arg-type* (info-foreign-arg-type* info)]
                                       ...
                                       [arg-cat* (categorize-arguments arg-type* #f)]
                                   ...
                                   (nanopass-case (Ltype Type) result-type
                                                  [(fp-double-float)
                                                   (lambda (lvalue) ; unboxed
                                                     `(inline ,(make-info-loadfl %Cfpretval) ,%store-double
                            ...
                            (let ()
                              ...
                              (define load-single-stack
                                (lambda (offset)
                                  (lambda (x) ; requires var
                                    (%seq
                                     (inline ,(make-info-loadfl %flreg2) ,%load-single->double ,%sp ,%zero (immediate ,offset))
                                     (inline ,(make-info-loadfl %flreg2) ,%store-double ,x ,%zero ,(%constant flonum-data-disp))))))
                                                                ...
                                                                (lambda (x offset)
                                                                  (let loop ([fp* (cat-regs result-cat)] [offset offset])
                                                                    (cond
                                                                     ...
                                                                     [double?
                                                                      (%seq ,(loop (cdr fp*) (fx+ offset 8))
                                                                            (inline ,(make-info-loadfl (car fp*))
                                                                                    ,%load-double ,x ,%zero (immediate ,offset)))]
                                                                     [else
                                                                      (%seq ,(loop (cdr fp*) (fx+ offset 4))
                                                                            (inline ,(make-info-loadfl (car fp*))
                                                                                    ,%load-single ,x ,%zero (immediate ,offset)))])))])
                                                           (if synthesize-first?
                                                               (lambda ()
                                                                 (to-regs %sp return-stack-offset))
                                                               (lambda (x)
                              ...
                              (lambda (info)
                                (define callee-save-regs+lr (cons* %lr
                                                                   ...
                                                                   %tc %sfp %ap %trap
                                                                   ;; allocable:
                                                                   (get-allocable-callee-save-regs gp-reg?)))
                                (define callee-save-fpregs  (get-allocable-callee-save-regs fp-reg?))
                                      ...
                                      (values
                                       (lambda ()
                                         (%seq
                                          ...
                                          ,(if (null? arg-fp-regs) `(nop) `(inline ,(make-info-kill*-live* '() arg-fp-regs) ,%push-multiple))
                                          ...
                                          (inline ,(make-info-kill*-live* '() callee-save-fpregs) ,%push-multiple)
                                          ;; maybe activate
                                          ,(if adjust-active?
                                               `(seq
                                       ...
                                       (lambda ()
                                         (in-context Tail
                                                     (%seq
                                                      ...
                                                      (inline ,(make-info-kill* callee-save-fpregs) ,%pop-multiple)
