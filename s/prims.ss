    ...
    [(n c)
     (unless (and (fixnum? n) (not ($fxu< (constant maximum-string-length) n)))
       (if (and (or (fixnum? n) (bignum? n))
                (> n (constant maximum-string-length)))
           ($impoops who "~s exceeds the maximum string length" n)
           ($oops who "~s is not a valid string length" n)))
     (unless (char? c)
   ...
   (case-lambda
      [(n x)
       (unless (and (fixnum? n) (not ($fxu< (constant maximum-vector-length) n)))
         (if (and (or (fixnum? n) (bignum? n))
                  (> n (constant maximum-vector-length)))
             ($impoops who "~s exceeds the maximum vector length" n)
